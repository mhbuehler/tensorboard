...
# ==============================================================================
"""Internal information about the custom plugin."""
...
from tensorboard.plugins.custom import plugin_data_pb2


PLUGIN_NAME = 'custom'

# The most recent value for the `version` field of the
# `CustomPluginData` proto.
PROTO_VERSION = 0


def create_summary_metadata(display_name, description):
  """Create a `tf.SummaryMetadata` proto for custom plugin data.
  Returns:
    A `tf.SummaryMetadata` protobuf object.
  """
  content = plugin_data_pb2.CustomPluginData(version=PROTO_VERSION)
...
def parse_plugin_metadata(content):
  ...
  Arguments:
    content: The `content` field of a `SummaryMetadata` proto corresponding to
      the custom plugin.
  Returns:
    A `CustomPluginData` protobuf object.
  """
  result = plugin_data_pb2.CustomPluginData()
